...
const Home: NextPage<HomeProps> = ({ notes: notesProp, todos: todosProp }) => {
  ...
  const selectAll = () => {
    switch(tab) {
      ...
      case 'todosTab':
        setSelectedCardIds(todos.map(todo => todo.id))
        break
          ...
          selectedCardIds.length > 0 ? (
            ...
            <nav className={styles.nav_bar}>
              <ul className={styles.nav_list} ref={navBarListRef} >
                <li
                 className={`${styles.nav_item} ${tab === 'notesTab' ? styles.active : ''}`} 
                 onClick={changeTab} id='notesTab'
                >
                  <Image src={readerIcon} alt='reader icon' />
                </li>
                <li
                 className={`${styles.nav_item} ${tab === 'todosTab' ? styles.active : ''}`} 
                 onClick={changeTab} id='todosTab'
                >
